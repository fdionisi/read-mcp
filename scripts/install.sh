...
download() {
  ...
  printf "\n" >&2
  info "This is likely due to read-mcp not yet supporting your configuration."
  info "If you would like to see a build for your configuration,"
  info "please create an issue requesting a build for ${MAGENTA}${TARGET}${NO_COLOR}:"
  info "${BOLD}${UNDERLINE}https://github.com/fdionisi/read-mcp/issues/new/${NO_COLOR}"
  return $rc
}
...
unpack() {
  ...
  info "bug report at https://github.com/fdionisi/read-mcp/issues"
  return 1
}
...
Fetch and install the latest version of read-mcp, if read-mcp is already
...
install() {
  ...
  if test_writeable "${BIN_DIR}"; then
    sudo=""
    msg="Installing read-mcp, please wait…"
  else
    warn "Escalated permissions are required to install to ${BIN_DIR}"
    elevate_priv
    sudo="sudo"
    msg="Installing read-mcp as root, please wait…"
  fi
  info "$msg"
...
is_build_available() {
  ...
  if [ "${good}" != "1" ]; then
    error "${arch} builds for ${platform} are not yet available for read-mcp"
    printf "\n" >&2
    info "If you would like to see a build for your configuration,"
    info "please create an issue requesting a build for ${MAGENTA}${target}${NO_COLOR}:"
    info "${BOLD}${UNDERLINE}https://github.com/fdionisi/read-mcp/issues/new/${NO_COLOR}"
...
if [ -z "${BASE_URL-}" ]; then
  BASE_URL="https://github.com/fdionisi/read-mcp/releases"
fi
...
URL="${BASE_URL}/latest/download/read-mcp-${TARGET}.${EXT}"
info "Tarball URL: ${UNDERLINE}${BLUE}${URL}${NO_COLOR}"
confirm "Install read-mcp ${GREEN}latest${NO_COLOR} to ${BOLD}${GREEN}${BIN_DIR}${NO_COLOR}?"
check_bin_dir "${BIN_DIR}"

install "${EXT}"
completed "read-mcp installed"
